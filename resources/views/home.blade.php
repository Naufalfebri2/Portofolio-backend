<x-layouts.portfolio title="Naufal Febriansyah — Backend Developer & Information Systems Student"
    description="Portfolio of Naufal Febriansyah, a Backend Developer focused on building SaaS systems and business applications using Laravel & PostgreSQL. Check out projects like KLolain, Employee Attendance App, and more.">
    ...
    <section
        ...
        @if ($profile && $profile->photo)
            <div class="relative w-32 h-32 mb-5 overflow-hidden rounded-full md:w-40 md:h-40">
        ...
        @if ($profile && $profile->location)
            <div class="flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 mb-6">
                <svg class="text-accent-500 dark:text-accent-400" width="14" height="14" viewBox="0 0 24 24"
        ...
        @php
            $roleText = $profile->role ?? 'Backend Developer';
            $roleWords = explode(' ', trim($roleText));
            $roleLastWord = array_pop($roleWords);
            $roleFirstPart = implode(' ', $roleWords);
        @endphp

        <h1 class="mb-6 text-4xl font-bold leading-tight text-gray-900 dark:text-white sm:text-6xl">
            @if ($roleFirstPart)
                {{ $roleFirstPart }}
            @endif
            <span class="text-transparent bg-gradient-accent bg-clip-text">{{ $roleLastWord }}</span>
        </h1>

        <p class="max-w-2xl mx-auto mb-10 leading-relaxed text-gray-600 dark:text-gray-400">
            Hi, I'm <span
                class="font-semibold text-gray-900 dark:text-white">{{ $profile->name ?? 'Naufal Febriansyah' }}</span>,
        ...
        <div class="flex items-center justify-center gap-4">
            ...
            <a href="{{ route('contact') }}"
                class="px-6 py-3 text-gray-900 transition border border-gray-300 rounded-lg dark:text-white dark:border-gray-700 btn-scale hover:border-gray-500">
                Contact Me
            </a>
        </div>
            ...
            <div class="fixed z-20 flex-col hidden gap-4 -translate-y-1/2 lg:flex right-8 top-1/2">
                @if ($profile->github_url)
                    <a href="{{ $profile->github_url }}" target="_blank" rel="noopener noreferrer" aria-label="GitHub"
                        class="text-gray-400 transition-colors dark:text-gray-500 hover:text-accent-500 dark:hover:text-accent-400">
    ...
    <section id="tentang" class="max-w-4xl px-4 py-16 mx-auto sm:px-6 lg:px-8 fade-in-up">
        <div class="mb-16 text-center">
            <p class="mb-2 text-sm font-medium text-accent-500 dark:text-accent-400">Behind The Scenes</p>
            <h2 class="text-3xl font-bold text-gray-900 dark:text-white sm:text-4xl">
                Experience <span class="text-transparent bg-gradient-accent bg-clip-text">Built</span>
            </h2>
        </div>
        ...
        <div class="relative pl-10">
            <div class="absolute left-[7px] top-0 bottom-0 w-px bg-gray-300 dark:bg-gray-800"></div>
            ...
            @foreach ($timeline as $item)
                <div class="relative mb-14 last:mb-0">
                    ...
                    <h3 class="mb-0.5 text-xl font-semibold text-gray-900 dark:text-white">{{ $item['role'] }}</h3>
                    <p class="mb-3 text-sm text-accent-500 dark:text-accent-400">{{ $item['org'] }}</p>
                    <p class="max-w-2xl mb-4 leading-relaxed text-gray-600 dark:text-gray-400">
                        {{ $item['description'] }}</p>

                    <div class="flex flex-wrap gap-2">
                        @foreach ($item['tags'] as $tag)
                            <span
                                class="px-2.5 py-1 text-xs text-gray-600 dark:text-gray-400 border border-gray-300 dark:border-gray-800 rounded-full">{{ $tag }}</span>
                        @endforeach
                    </div>
                </div>
            @endforeach
    ...
    <section id="stacks" class="max-w-5xl px-4 py-16 mx-auto sm:px-6 lg:px-8 fade-in-up">
        <div class="mb-10 text-center">
            <p class="mb-2 text-sm font-medium text-accent-500 dark:text-accent-400">Built With Precision</p>
            <h2 class="text-3xl font-bold text-gray-900 dark:text-white sm:text-4xl">
                Core <span class="text-transparent bg-gradient-accent bg-clip-text">Stack</span>
            </h2>
        </div>
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap justify-center gap-3 md:justify-start">
                            @foreach ($stacks[$categoryName] as $tech)
                                ...
                                <div
                                    class="flex items-center gap-2.5 px-4 py-2.5 rounded-xl border border-gray-300 dark:border-gray-800 bg-gray-100 dark:bg-gray-900/40 hover:border-accent-500/60 transition-colors">
                                    @if ($iconData)
                                        <img src="https://cdn.simpleicons.org/{{ $iconData['slug'] }}/{{ $iconData['color'] }}"
                                            alt="{{ $tech->name }}" class="w-[18px] h-[18px]" loading="lazy">
                                    @endif
                                    <span class="text-sm text-gray-700 dark:text-gray-300">{{ $tech->name }}</span>
                                </div>
                            @endforeach
                        </div>
                    </div>
    ...
    <section id="project" class="max-w-5xl px-4 py-16 mx-auto sm:px-6 lg:px-8 fade-in-up">
        <div class="mb-10 text-center">
            <p class="mb-2 text-sm font-medium text-accent-500 dark:text-accent-400">From Idea to System</p>
            <h2 class="text-3xl font-bold text-gray-900 dark:text-white sm:text-4xl">
                Featured <span class="text-transparent bg-gradient-accent bg-clip-text">Projects</span>
            </h2>
        </div>
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-3">
                @foreach ($featuredProjects as $project)
                    <a href="{{ route('projects.show', $project->slug) }}"
                        class="block overflow-hidden transition bg-gray-100 border border-gray-300 dark:bg-gray-900 dark:border-gray-800 hover-lift rounded-xl hover:border-accent-500/60">
                        @if ($project->thumbnail)
                            <img src="{{ asset('storage/' . $project->thumbnail) }}" alt="{{ $project->title }}"
                                class="object-cover object-top w-full h-40">
                        @else
                            <div
                                class="flex items-center justify-center w-full h-40 text-gray-500 bg-gray-200 dark:bg-gray-800">
                                No Image
                            </div>
                        @endif
                        <div class="p-5">
                            <h3 class="mb-2 font-semibold text-gray-900 dark:text-white">{{ $project->title }}</h3>
                            <p class="mb-3 text-sm text-gray-600 dark:text-gray-400 line-clamp-2">
                                {{ $project->description }}</p>
                            <div class="flex flex-wrap gap-2">
                                @foreach ($project->technologies->take(3) as $tech)
                                    <span
                                        class="px-2 py-1 text-xs text-gray-700 bg-gray-200 rounded dark:text-gray-300 dark:bg-gray-800">{{ $tech->name }}</span>
                                @endforeach
                            </div>
                        </div>
                    </a>
    ...
    <section class="max-w-4xl px-4 py-16 mx-auto text-center sm:px-6 lg:px-8 fade-in-up">
        <div
            class="p-10 border border-gray-300 rounded-2xl dark:border-gray-800 bg-gradient-to-br from-accent-500/10 to-cyan-400/10">
            <p class="mb-2 text-sm font-medium text-accent-500 dark:text-accent-400">Have a Backend Project?</p>
            <h2 class="mb-4 text-2xl font-bold text-gray-900 dark:text-white sm:text-3xl">
                Let's Discuss Your System Requirements
            </h2>
            <p class="max-w-lg mx-auto mb-8 leading-relaxed text-gray-600 dark:text-gray-400">
